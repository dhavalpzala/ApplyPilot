...
ApplyPilot is a 6-stage autonomous job application pipeline. It discovers jobs across 5+ boards, scores them against your resume with AI, attaches the resume you supplied at setup, writes a cover letter per job, and **submits applications for you**. It navigates forms, uploads documents, answers screening questions, all hands-free.
...
applypilot run           # discover > enrich > score > attach resume > cover letters
...
### Discovery + Cover Letters Only
**Requires:** Python 3.11+, Gemini API key (free)

Runs stages 1-5: discovers jobs, scores them, attaches your resume, generates cover letters. You submit applications manually with the prepared materials.
...
| **4. Tailor** | Attaches the resume you supplied at setup to every qualifying job. Same document every time, no LLM |
...
| Cover letters | Per-job AI draft | Template-based | Hours per application |
...
| Gemini API key *or* a local model | Scoring, cover letters, auto-apply | Free tier (15 RPM / 1M tokens/day) is enough; see "Running fully local" below |
...
### `profile.json`
Your personal data in one structured file: contact info, work authorization, compensation, experience, skills, resume facts (checked against AI-written cover letters), and EEO defaults. Powers scoring, cover letters, and form auto-fill.
...
### Score
AI scores every job 1-10 against your profile. 9-10 = strong match, 7-8 = good, 5-6 = moderate, 1-4 = skip. Only jobs above your threshold proceed to the rest of the pipeline.

### Tailor
Attaches the resume you supplied during `applypilot init` to every job above your score threshold. The same document is uploaded with every application — it is never rewritten per job. No LLM calls, so this stage is instant. The `.pdf` is what gets uploaded; the `.txt` companion is what the apply agent reads when filling text fields.

### Cover Letter
Writes a targeted cover letter per job referencing the specific company, role, and how your experience maps to their requirements.

### Auto-Apply
Claude Code launches a Chrome instance, navigates to each application page, detects the form type, fills personal information and work history, uploads your resume and the cover letter, answers screening questions with AI, and submits. A live dashboard shows progress in real-time.
